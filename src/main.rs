...
enum Commands {
    ...
    /// Probe which firmware command opcodes the Operator acknowledges (experimental).
    ///
    /// A diagnostic for reverse-engineering the protocol: it sends each command byte
    /// in a range and reports whether the firmware answers with a READY frame. It does
    /// NOT complete any data phase, so it neither reads nor writes a cartridge. Remove
    /// the cartridge before running. Destructive opcodes (DFU reboot, factory reset,
    /// the write commands) are always skipped.
    #[command(hide = true)]
    Probe {
        /// Opcodes to scan: comma-separated singles or ranges, hex or decimal
        /// (e.g. "0x06-0x1f", "0x14,0x16-0x1f", "6-31")
        #[arg(long, default_value = "0x06-0x1f")]
        range: String,
        /// Also probe the known/documented opcodes (skipped by default)
        #[arg(long)]
        include_known: bool,
        /// Argument byte 1 (the chip-type slot, packet[1])
        #[arg(long, default_value_t = 0)]
        a1: u8,
        /// Argument 2 (the rom-size slot, packet[2..6], u32-LE) — hex or decimal
        #[arg(long, default_value_t = 0, value_parser = parse_u32)]
        a2: u32,
        /// Argument 3 (the save-size slot, packet[6..10], u32-LE) — hex or decimal
        #[arg(long, default_value_t = 0, value_parser = parse_u32)]
        a3: u32,
        /// Dump the raw response bytes (hex) for every opcode
        #[arg(long)]
        raw: bool,
...
/// Parse a u32 from hex (`0x..`) or decimal, for clap value parsing.
...
fn main() {
    ...
    match command {
        ...
        Commands::Probe { range, include_known, a1, a2, a3, raw, yes } => {
            run_probe(&range, include_known, a1, a2, a3, raw, yes);
        }
    }
}

/// Opcodes that are never probed: they reboot the device, reset it, or write to the
/// cartridge. Sending any of these does real (and in the DFU cases, disconnecting)
/// harm, so they are filtered out regardless of the requested range.
const PROBE_DANGEROUS: [u8; 7] = [
    0x01, // WriteGame
    0x03, // WriteSave
    0x05, // RebootSysDfu
    0x08, // SettingsWriteLed
    0x10, // WriteRTC
    0x20, // FactoryReset
    0x35, // RebootAppDfu
];

/// Opcodes Playback is known to use. Skipped by default (we already know they exist);
/// `--include-known` probes them anyway to confirm framing.
const PROBE_KNOWN: [u8; 8] = [
    0x00, // ReadGame
    0x02, // ReadSave
    0x04, // ReadSignature
    0x09, // ReadRTC
    0x11, // StartPeripheral
    0x12, // UsePeripheral
    0x13, // StopPeripheral
    0x15, // DetectFlashcart
];

/// Parse an opcode range spec: comma-separated tokens, each either a single value or
/// an inclusive `a-b` range, in hex (`0x..`) or decimal.
fn parse_opcode_range(spec: &str) -> Result<Vec<u8>, String> {
    let parse_one = |s: &str| -> Result<u8, String> {
        ...
        .map_err(|_| format!("invalid opcode '{s}'"))?;
        u8::try_from(val).map_err(|_| format!("opcode '{s}' out of range (0-255)"))
    };
    ...
    for token in spec.split(',') {
        ...
        match token.split_once('-') {
            Some((a, b)) => {
                ...
                if start > end {
                    return Err(format!("range '{token}' is backwards"));
                }
                out.extend(start..=end);
            }
            None => out.push(parse_one(token)?),
        }
    }
    if out.is_empty() {
        return Err("no opcodes to probe".into());
    }
    out.sort_unstable();
    out.dedup();
    Ok(out)
...
#[allow(clippy::fn_params_excessive_bools)]
fn run_probe(range: &str, include_known: bool, a1: u8, a2: u32, a3: u32, raw: bool, yes: bool) {
    let requested = parse_opcode_range(range).unwrap_or_else(|e| {
        eprintln!("Error: {e}");
        process::exit(1);
    });

    let mut skipped_dangerous = Vec::new();
    let mut skipped_known = Vec::new();
    let mut to_probe = Vec::new();
    for op in requested {
        if PROBE_DANGEROUS.contains(&op) {
            skipped_dangerous.push(op);
        } else if PROBE_KNOWN.contains(&op) && !include_known {
            skipped_known.push(op);
        } else {
            to_probe.push(op);
        }
    }

    if !skipped_dangerous.is_empty() {
        eprintln!(
            "Skipping destructive opcodes: {}",
            skipped_dangerous.iter().map(|b| format!("0x{b:02X}")).collect::<Vec<_>>().join(", ")
        );
    }
    if !skipped_known.is_empty() {
        eprintln!(
            "Skipping known opcodes (pass --include-known to probe them): {}",
            skipped_known.iter().map(|b| format!("0x{b:02X}")).collect::<Vec<_>>().join(", ")
        );
    }
    if to_probe.is_empty() {
        eprintln!("Nothing left to probe.");
        process::exit(1);
    }

    eprintln!(
        "About to probe {} opcode(s): {}",
        to_probe.len(),
        to_probe.iter().map(|b| format!("0x{b:02X}")).collect::<Vec<_>>().join(", ")
    );
    eprintln!(
        "This sends raw command bytes to the Operator to see which it acknowledges. \
         No data phase is completed, so no cartridge read or write occurs. Remove the \
         cartridge first as a precaution."
    );
    ...
    if a1 != 0 || a2 != 0 || a3 != 0 {
        eprintln!("Args: a1=0x{a1:02X} a2=0x{a2:08X} a3=0x{a3:08X}");
    }

    // Count payload bytes = everything that isn't part of a C0 DE framing packet or
    // zero padding. A raw read primitive would show a large non-frame payload here.
    let payload_len = |raw: &[u8]| -> usize {
    ...
    for op in &to_probe {
        match device::probe_opcode(*op, a1, a2, a3) {
            Ok(r) => {
                let pl = payload_len(&r.raw);
                // A no-op opcode still returns ~18 non-zero framing bytes (READY + DONE
                // frames + lead-in markers). Real returned data pushes well above that;
                // ReadSignature (0x04) measured 37. Flag only clear data payloads.
                let has_payload = pl > 24;
                if has_payload {
                    interesting.push(*op);
                }
                let note = if r.ready && r.done {
                    acknowledged.push(*op);
                    format!("READY+DONE  ({} bytes back, {pl} non-zero)", r.raw.len())
                } else if r.ready {
                    acknowledged.push(*op);
                    format!("READY       ({} bytes back, {pl} non-zero)", r.raw.len())
                } else if r.done {
                    acknowledged.push(*op);
                    format!("DONE        ({} bytes back, {pl} non-zero)", r.raw.len())
                } else if let Some(f) = r.other_frame {
                    format!("frame       C0 DE {:02X} {:02X} {:02X} (unexpected)", f[2], f[3], f[4])
                } else if r.raw.iter().any(|&b| b != 0) {
                    format!("data        {} bytes ({pl} non-zero)", r.raw.len())
                } else {
                    "no response".to_string()
                };
                println!("  0x{op:02X}  {note}{}", if has_payload { "  <-- PAYLOAD" } else { "" });
                if raw {
                    let hex: Vec<String> =
                        r.raw.iter().take(64).map(|b| format!("{b:02X}")).collect();
                    println!("         raw: {}{}", hex.join(" "), if r.raw.len() > 64 { " ..." } else { "" });
                }
            }
            Err(e) => {
                println!("  0x{op:02X}  error: {e}");
            }
        }
    }
    ...
    if !interesting.is_empty() {
        eprintln!(
            "\nReturned a data payload (candidate read/query opcodes): {}",
            interesting.iter().map(|b| format!("0x{b:02X}")).collect::<Vec<_>>().join(", ")
        );
    }

    eprintln!();
    if acknowledged.is_empty() {
        eprintln!("No new opcodes acknowledged in this range.");
    } else {
        eprintln!(
            "Acknowledged: {}",
            acknowledged.iter().map(|b| format!("0x{b:02X}")).collect::<Vec<_>>().join(", ")
        );
        eprintln!("These are candidates for further study.");
    }
}
