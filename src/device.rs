...
/// Result of probing a single command opcode (see `probe_opcode`).
pub struct ProbeResult {
    /// The opcode that was sent.
    pub opcode: u8,
    /// Firmware answered with a READY frame `C0 DE 00 <op> <~op>` — it recognises
    /// this opcode and is waiting for the data phase.
    pub ready: bool,
    /// Firmware answered with a DONE frame `C0 DE 01 <op> <~op>` — the command ran
    /// to completion.
    pub done: bool,
    /// A `C0 DE` frame came back carrying a *different* command byte (a NAK, or an
    /// unrelated queued frame). None if no such frame was seen.
    pub other_frame: Option<[u8; 5]>,
    /// Every byte received during the probe (for inspection / hex dump).
    pub raw: Vec<u8>,
}

/// Look for a 5-byte framing packet `C0 DE <kind> <cmd> <~cmd>` anywhere in `buf`.
fn has_frame(buf: &[u8], kind: u8, cmd: u8) -> bool {
    buf.windows(5).any(|w| {
        w[0] == 0xC0 && w[1] == 0xDE && w[2] == kind && w[3] == cmd && w[4] == !cmd
    })
}

/// Find the first `C0 DE` frame whose command byte is *not* `expected_cmd`.
fn find_other_frame(buf: &[u8], expected_cmd: u8) -> Option<[u8; 5]> {
    buf.windows(5)
        .find(|w| w[0] == 0xC0 && w[1] == 0xDE && w[3] != expected_cmd)
        .map(|w| [w[0], w[1], w[2], w[3], w[4]])
}

/// Send a single raw command opcode to the connected Operator and report what comes
/// back, without attempting any data phase. This is a low-level diagnostic used to
/// enumerate which command bytes the firmware acknowledges — it does not know or care
/// what an opcode *does*, only whether the firmware answers with a READY/DONE frame.
///
/// A fresh serial connection is opened per call (re-asserting DTR/RTS) so each probe
/// starts from clean device state; after reading the response a zero ACK packet is
/// sent to unstick any opcode that entered a data phase, and the buffer is drained.
///
/// The caller is responsible for only passing opcodes that are safe to send. Sending
/// a write/erase/reboot opcode will do exactly that.
///
/// `arg1`/`arg2`/`arg3` fill the command packet's argument fields (byte 1, and the two
/// u32-LE fields at 2..6 and 6..10) — the same slots ReadGame etc. use for chip type,
/// ROM size and save size. Passing non-zero args lets a caller test whether an opcode
/// behaves like a read (streams data back) or acts on an address.
pub fn probe_opcode(opcode: u8, arg1: u8, arg2: u32, arg3: u32) -> Result<ProbeResult, DeviceError> {
    let (port_name, _kind) = find_operator()?;
    let mut io = Serial::open(&port_name)?;
    io.flush_input();

    // Short per-probe timeout so a full scan doesn't take minutes; an opcode the
    // firmware ignores just yields silence for one timeout window.
    io.port.set_timeout(Duration::from_millis(350))?;

    let mut packet = [0u8; PACKET_SIZE];
    packet[0] = opcode;
    packet[1] = arg1;
    packet[2..6].copy_from_slice(&arg2.to_le_bytes());
    packet[6..10].copy_from_slice(&arg3.to_le_bytes());
    let checksum = CRC.checksum(&packet[..60]);
    packet[60..64].copy_from_slice(&checksum.to_le_bytes());
    io.send(&packet)?;

    let mut raw = io.read_until_timeout();
    // If the opcode was recognised it may now be waiting for a data phase; a zero
    // packet lets it complete or error out rather than leaving the port mid-frame.
    let _ = io.send_ack();
    raw.extend(io.read_until_timeout());

    let ready = has_frame(&raw, 0x00, opcode);
    let done = has_frame(&raw, 0x01, opcode);
    let other_frame = if ready || done { None } else { find_other_frame(&raw, opcode) };

    Ok(ProbeResult { opcode, ready, done, other_frame, raw })
}
